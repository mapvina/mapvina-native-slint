# MapVina Native + Slint Reference Implementation

This repository is a working reference for using [MapVina Native](https://github.com/mapvina/mapvina-native) inside [Slint](https://slint.dev/) applications.
...
- A reusable Slint component library centered on [`src/mapvina.slint`](src/mapvina.slint)
- A canonical C++ backend integration that works on Linux, Windows, and macOS
- A practical reference for people who want to build their own Slint + MapVina app
...
> If you want to build a Slint application that embeds MapVina, this repository shows a real cross-platform way to do it.
...
The Rust demo exists to mirror the same Slint component contract, but it depends on [`mapvina-native-rs`](https://github.com/mapvina/mapvina-native-rs), which is currently only practical on Linux. Treat it as an experimental companion, not the primary integration path.
...
```bash
git clone https://github.com/mapvina/mapvina-native-slint.git
cd mapvina-native-slint
git submodule update --init --recursive

cmake -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build

./build/cpp/mapvina-slint-example
```
...
The public Slint entrypoint is [`src/mapvina.slint`](src/mapvina.slint):

```slint
import { MMapView, MMapAdapter } from "@mapvina-native-slint/mapvina.slint";
```
...
```slint
import { MMapView } from "@mapvina-native-slint/mapvina.slint";
...
export component App inherits Window {
    ...
    map := MMapView {
        style-url: "https://maps.mapvina.com/styles/v1/streets.json?key=public_key";
        center-lat: 35.6895;
        center-lon: 139.6917;
        zoom: 10;
    }
...
- [`src/mapvina.slint`](src/mapvina.slint) is the public entrypoint

### Canonical backend

- [`cpp/main.cpp`](cpp/main.cpp) wires `MMapAdapter` to the native map engine
- [`cpp/src/slint_mapvina_headless.cpp`](cpp/src/slint_mapvina_headless.cpp) holds the current production-grade backend logic
...
- [`rust/src/mapvina.rs`](rust/src/mapvina.rs) wires `MMapAdapter` to `mapvina-native-rs`
...
| Windows x64 | Not practical | Blocked by `mapvina-native-rs` maturity |
| macOS Apple Silicon | Not practical | Blocked by `mapvina-native-rs` maturity |
...
1. MapVina Native renders into its headless frontend using the selected backend
...
- [`vendor/`](vendor/) - MapVina Native and other vendored dependencies
...
- MapVina Native Slack: [#mapvina-native](https://osmus.slack.com/archives/C01G4G39862)
- OSM US Slack invite: [slack.openstreetmap.us](https://slack.openstreetmap.us/)
- MapVina website: [mapvina.com](https://mapvina.com/)
