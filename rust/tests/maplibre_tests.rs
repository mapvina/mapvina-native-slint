...
// This test file tests the headless MapVina functionality
...
fn test_url_parsing() {
    ...
    let valid_url = "https://maps.mapvina.com/styles/v1/streets.json?key=public_key";
...
fn test_cache_path() {
    ...
    let cache_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("mapvina_database.sqlite");

    // Check that the path has the correct filename
    assert_eq!(cache_path.file_name().unwrap(), "mapvina_database.sqlite");
