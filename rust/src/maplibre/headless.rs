...
const DEFAULT_STYLE_URL: &str = "https://maps.mapvina.com/styles/v1/streets.json?key=public_key";
...
pub struct MapVina {
    renderer: ImageRenderer<mapvina_native::Static>,
    last_image: Option<mapvina_native::Image>,
    size: (u32, u32),
    style_url: String,
    camera: MapCamera,
...
impl MapVina {
    pub fn new(renderer: ImageRenderer<mapvina_native::Static>, size: (u32, u32)) -> Self {
        Self {
            renderer,
            last_image: None,
            size,
    ...
    pub fn read_still_image(&mut self) -> &mapvina_native::Image {
...
fn cache_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("mapvina_database.sqlite")
}
...
fn build_renderer(size: (u32, u32), style_url: &str) -> ImageRenderer<mapvina_native::Static> {
...
pub fn create_map(size: Size) -> Rc<RefCell<MapVina>> {
    ...
    Rc::new(RefCell::new(MapVina::new(renderer, size)))
}
...
mod tests {
    ...
    fn drag_changes_camera_state_in_expected_direction() {
        let mut map = MapVina::new(build_renderer((256, 256), DEFAULT_STYLE_URL), (256, 256));
