...
void SlintMapVina::initialize(int w, int h) {
    width = w;
    height = h;

    std::cout << "[SlintMapVina] initialize(" << w << "," << h << ")"
              << std::endl;
    ...
    // Try remote MapVina demo style first; fall back to local JSON on error
    std::cout << "Loading remote MapVina style..." << std::endl;
    map->getStyle().loadURL("https://maps.mapvina.com/styles/v1/streets.json?key=public_key");
    ...
    std::cout << "[SlintMapVina] Map initialization completed" << std::endl;
}

void SlintMapVina::setRenderCallback(std::function<void()> callback) {
    m_renderCallback = std::move(callback);
}

// MapObserver implementation
void SlintMapVina::onWillStartLoadingMap() {
    std::cout << "[MapObserver] Will start loading map" << std::endl;
    style_loaded = false;
    map_idle = false;
}

void SlintMapVina::onDidFinishLoadingStyle() {
    std::cout << "[MapObserver] Did finish loading style" << std::endl;
    style_loaded = true;
}

void SlintMapVina::onDidBecomeIdle() {
    std::cout << "[MapObserver] Did become idle" << std::endl;
    map_idle = true;
}

void SlintMapVina::onDidFailLoadingMap(mbgl::MapLoadError error,
...
void SlintMapVina::onCameraDidChange(CameraChangeMode) {
    request_repaint();
    arm_forced_repaint_ms(100);
}

void SlintMapVina::onSourceChanged(mbgl::style::Source&) {
    request_repaint();
    arm_forced_repaint_ms(100);
}

void SlintMapVina::onDidFinishRenderingFrame(const RenderFrameStatus& status) {
...
void SlintMapVina::setStyleUrl(const std::string& url) {
    if (map) {
        map->getStyle().loadURL(url);
    }
}

slint::Image SlintMapVina::render_map() {
...
void SlintMapVina::resize(int w, int h) {
    width = w;
    height = h;
...
void SlintMapVina::handle_mouse_press(float x, float y) {
    last_pos = {x, y};
...
void SlintMapVina::handle_mouse_release(float x, float y) {
    // No action needed for release
}

void SlintMapVina::handle_mouse_move(float x, float y, bool pressed) {
    if (pressed) {
...
void SlintMapVina::handle_double_click(float x, float y, bool shift) {
    if (!map)
        return;
...
void SlintMapVina::handle_wheel_zoom(float x, float y, float dy) {
    if (!map)
        return;
...
void SlintMapVina::set_pitch(int pitch_value) {
    if (!map)
        return;
...
void SlintMapVina::set_bearing(float bearing_value) {
    if (!map)
        return;
...
void SlintMapVina::run_map_loop() {
    if (run_loop) {
        run_loop->runOnce();
    } else {
...
void SlintMapVina::arm_forced_repaint_ms(int ms) {
...
void SlintMapVina::fly_to(double lat, double lon, double target_zoom_value) {
    if (!map)
        return;
...
void SlintMapVina::fly_to(const std::string& location) {
    if (!map)
        return;
...
void SlintMapVina::tick_animation() {
