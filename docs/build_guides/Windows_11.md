...
This guide walks you through building the canonical C++ reference application for `mapvina-native-slint` on Windows 11 with MSVC, CMake, Ninja, and vcpkg (manifest mode).
...
set VCPKG_OVERLAY_TRIPLETS=%cd%\vendor\mapvina-native\platform\windows\vendor\vcpkg-custom-triplets
```
...
> ```bat
> set LIBCLANG_PATH=C:\Program Files\LLVM\lib
> cd vendor\mapvina-native\vendor\wgpu-native
> cargo build --release
> cd ..\..\..\..
> ```
...
- `build-ninja\cpp\mapvina-slint-example.exe` — Example application
...
- The DLL is automatically copied to the output directory during the CMake build.
  ...
  ```bat
  copy vendor\mapvina-native\vendor\wgpu-native\target\release\wgpu_native.dll build-ninja\cpp\
  ```
...
- First build can take a while (MapVina Native + Slint + optionally wgpu-native).
...
1. Start the sample: `mapvina-slint-example.exe`
2. Read `src/mapvina.slint` for the reusable Slint surface
